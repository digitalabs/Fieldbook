...
/**
 * This is the abstract class that implements basic calls to client RESTful web services.
 */
...
public abstract class AbstractRestfulService {
	...
	public <T> T get(String url, Class<T> responseType, Object... param) {
		return this.restTemplate.getForObject(url, responseType, param);
	}
	...
	public <T> List<T> getList(String url, Class<T> responseType) {
		T[] results = (T[]) this.restTemplate.getForObject(url, Array.newInstance(responseType, 0).getClass());
	...
	public <T> List<T> getList(String url, Class<T> responseType, Object... param) {
		T[] results = (T[]) this.restTemplate.getForObject(url, Array.newInstance(responseType, 0).getClass(), param);
		return Arrays.asList(results);
	}
}
