...
public class StudyTreeController {
	...
	public Map<String, Object> addStudyFolder(final HttpServletRequest req) {
		...
		try {
			...
			if (this.studyDataManager.isStudy(parentFolderId)) {
				final DmsProject project = this.studyDataManager.getParentFolder(parentFolderId);
				if (project == null) {
			...
			final int newFolderId =
					this.studyDataManager.addSubFolder(parentFolderId, folderName, folderName, this.getCurrentProgramUUID());
			resultsMap.put(StudyTreeController.IS_SUCCESS, "1");
	...
	public Map<String, Object> moveStudyFolder(final HttpServletRequest req) throws MiddlewareQueryException {
