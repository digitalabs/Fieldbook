...
public class ExpDesignUtil {
	...
	public static List<MeasurementRow> generateExpDesignMeasurements(int environments, int environmentsToAdd,
		...
		for (int trialNo = trialInstanceStart; trialNo <= environments; trialNo++) {
			...
			for (int counter = 0; counter < bvOutput.getBvResultList().size(); counter++) {
				...
				MeasurementRow row =
						ExpDesignUtil.createMeasurementRow(varList, importedGermplasm.get(), bvOutput.getEntryMap(counter),
	...
	private static Optional<ImportedGermplasm> findImportedGermplasmByEntryNumberAndChecks(
	...
	private static Optional<ImportedGermplasm> findImportedGermplasmByEntryNumberGeneratedByDesignEngine(
			Map<Integer, ImportedGermplasm> importedGermplasmMap, Integer entryNumber, Map<Integer, Integer> mapOfChecks) {
	 ...
	 * 
	 */
	private static Optional<Integer> resolveMappedEntryNumber(Map<Integer, ImportedGermplasm> importedGermplasmMap,
