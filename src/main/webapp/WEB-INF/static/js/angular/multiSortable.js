...
angular.module('ui.sortable').directive('uiMultiSortable', ['uiSortableConfig', '$parse', function(uiConfig, $parse) {
    ...
    var ModelSynchronizer = function(uiElement, attrs) {
      ...
      // Daniel Villafuerte
      // expose model for callback use
      this.embedTargetObject = function(model, ui) {
          if(attrs.modelSubset === undefined) {
              ui.targetObject = model[self.data.destPosition];
          } else {
              ui.targetObject = ($parse(self.data.destSubset)(model))[self.data.destPosition];
          }
      };
    };
      ...
      link: function(scope, element, attrs, ngModel) {
        ...
        if (ngModel !== null) {
          ...
          opts.stop = function(e, ui) {
            ...
            modelSync.embedTargetObject(ngModel.$modelValue, ui);
