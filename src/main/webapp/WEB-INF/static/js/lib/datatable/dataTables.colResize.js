...
(function (window, document, undefined) {
    ...
    $.fn.DataTable.Api.register('colResize.initWait()', function (settings) {
        var colResize = new dt.ColResize(this, settings);
        if (this.settings()[0]._bInitComplete)
            colResize.initialize();
        else{        	
            this.one('init.dt', function () {
                setTimeout(function(){colResize.initialize();}, 1500);
            });
        }
        return null;
    });
    ...
    $.fn.dataTable.ext.feature.push({
        "fnInit": function (oSettings) {
            ...
            return oSettings.oInstance.api().colResize.initWait(oSettings.oInit.colResize);
        },
        "cFeature": "J",
        "sFeature": "ColResize"
    });
