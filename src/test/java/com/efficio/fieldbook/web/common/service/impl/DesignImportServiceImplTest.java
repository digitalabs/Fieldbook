...
public class DesignImportServiceImplTest {
	...
	private Workbook workbook = WorkbookDataUtil.getTestWorkbookForTrial(10, 3);
	...
	public void testAddGermplasmDetailsToDataList() throws FileParsingException{
		...
		List<ImportedGermplasm> importedGermplasm = createImportedGermplasmList();
		Map<Integer,StandardVariable> germplasmStandardVariables = getStandardVariables(PhenotypicType.GERMPLASM ,workbook.getFactors());
	    List<MeasurementData> dataList = new ArrayList<>();
		
		service.addGermplasmDetailsToDataList(importedGermplasm, germplasmStandardVariables, dataList, 1);
		
		Assert.assertEquals("The added MeasurementData should Match the germplasm Standard Variables",germplasmStandardVariables.size(), dataList.size());
		...
		for(MeasurementData measurementData :dataList){
			...
			if (TermId.ENTRY_NO.getId() == measurementData.getMeasurementVariable().getTermId()){
				Assert.assertEquals("The value of MeasurementData should match the germplasm value : " + TermId.ENTRY_NO.toString(), measurementData.getValue().toString(), germplasmEntry.getEntryId().toString());
			}
			if (TermId.GID.getId() == measurementData.getMeasurementVariable().getTermId()){
				Assert.assertEquals("The value of MeasurementData should match the germplasm value : " + TermId.GID.toString(), measurementData.getValue().toString(), germplasmEntry.getGid().toString());
			}
			if (TermId.DESIG.getId() == measurementData.getMeasurementVariable().getTermId()){
				Assert.assertEquals("The value of MeasurementData should match the germplasm value : " + TermId.DESIG.toString(), measurementData.getValue().toString(), germplasmEntry.getDesig().toString());
			}
			if (TermId.ENTRY_TYPE.getId() == measurementData.getMeasurementVariable().getTermId()){
				Assert.assertEquals("The value of MeasurementData should match the germplasm value : " + TermId.ENTRY_TYPE.toString(), measurementData.getValue().toString(), germplasmEntry.getCheck().toString());
			}
			if (TermId.CROSS.getId() == measurementData.getMeasurementVariable().getTermId()){
				Assert.assertEquals("The value of MeasurementData should match the germplasm value : " + TermId.CROSS.toString(), measurementData.getValue().toString(), germplasmEntry.getCross().toString());
			}
			if (TermId.ENTRY_CODE.getId() == measurementData.getMeasurementVariable().getTermId()){
				Assert.assertEquals("The value of MeasurementData should match the germplasm value : " + TermId.ENTRY_CODE.toString(), measurementData.getValue().toString(), germplasmEntry.getEntryCode().toString());
			}
			if (TermId.GERMPLASM_SOURCE.getId() == measurementData.getMeasurementVariable().getTermId()){
				Assert.assertEquals("The value of MeasurementData should match the germplasm value : " + TermId.GERMPLASM_SOURCE.toString(), measurementData.getValue().toString(), germplasmEntry.getSource().toString());
			}
			if (TermId.SEED_SOURCE.getId() == measurementData.getMeasurementVariable().getTermId()){
				Assert.assertEquals("The value of MeasurementData should match the germplasm value : " + TermId.SEED_SOURCE.toString(), measurementData.getValue().toString(), germplasmEntry.getSource().toString());
			}
	...
	public void testAddVariatesToMeasurementRows() throws DesignValidationException, MiddlewareQueryException{
		...
		doReturn(createImportedGermplasmMainInfo()).when(userSelection).getImportedGermplasmMainInfo();
		
		EnvironmentData environmentData = createEnvironmentData(1);
		
		processEnvironmentData(environmentData);
		createDesignHeaderItemMap(designImportData);
		
		List<MeasurementRow> measurements = service.generateDesign(workbook, designImportData, environmentData);
		...
		service.addVariatesToMeasurementRows(workbook, measurements);
		
		Assert.assertEquals("The size of the data list should be 12 since 2 variates are added",12, measurements.get(0).getDataList().size());
	...
	public void testAreTrialInstancesMatchTheSelectedEnvironments() throws MiddlewareQueryException{
		...
		Assert.assertFalse("Should be false because the no of environments in trials don't match the no of trials in the design file", service.areTrialInstancesMatchTheSelectedEnvironments(1, designImportData));
		Assert.assertTrue("Should be true because the no of environments in trials match the no of trials in the design file", service.areTrialInstancesMatchTheSelectedEnvironments(3, designImportData));
	}
	
	@Test
	public void testCategorizeHeadersByPhenotype() throws MiddlewareQueryException{
		
		Map<PhenotypicType, List<DesignHeaderItem>> result = service.categorizeHeadersByPhenotype(designImportData.getUnmappedHeaders());
	...
	public void testConvertToStandardVariables(){
		...
		Map<Integer, StandardVariable> result = service.convertToStandardVariables(workbook.getGermplasmFactors());
		
		Assert.assertEquals(result.size(), workbook.getGermplasmFactors().size());
		
		for (MeasurementVariable measurementVar: workbook.getFactors()){
	...
	public void testCreateMeasurementData(){
		...
		MeasurementVariable measurementVariable = workbook.getFactors().get(0);
		MeasurementData data = service.createMeasurementData(measurementVariable, "1");
	...
	public void testExtractTrialInstancesFromEnvironmentData(){
		...
		EnvironmentData environmentData = createEnvironmentData(5);
		processEnvironmentData(environmentData);
		
		Set<String> result = service.extractTrialInstancesFromEnvironmentData(environmentData);
	...
	public void testFilterDesignHeaderItemsByTermId() throws MiddlewareQueryException{
		...
		List<DesignHeaderItem> headerDesignItems = designImportData.getMappedHeaders().get(PhenotypicType.TRIAL_ENVIRONMENT);
		DesignHeaderItem result = service.filterDesignHeaderItemsByTermId(TermId.TRIAL_INSTANCE_FACTOR, headerDesignItems);
	...
	public void testGenerateDesignForThreeInstances() throws MiddlewareQueryException, DesignValidationException{
		...
		EnvironmentData environmentData = createEnvironmentData(3);
	...
	public void testGetDesignMeasurementVariables() throws MiddlewareQueryException{
		...
		createDesignHeaderItemMap(designImportData);
		Set<MeasurementVariable> result = service.getDesignMeasurementVariables(workbook, designImportData);
	...
	public void testGetDesignRequiredMeasurementVariable() throws MiddlewareQueryException{
		...
		createDesignHeaderItemMap(designImportData);
		Set<MeasurementVariable> result = service.getDesignRequiredMeasurementVariable(workbook, designImportData);
	...
	public void testGetDesignRequiredStandardVariables() throws MiddlewareQueryException{
		...
		createDesignHeaderItemMap(designImportData);
		Set<StandardVariable> result = service.getDesignRequiredStandardVariables(workbook, designImportData);
	...
	public void testGroupCsvRowsIntoTrialInstance() throws MiddlewareQueryException{
		...
		Map<String, Map<Integer, List<String>>> result = service.groupCsvRowsIntoTrialInstance(trialInstanceHeaderItem, designImportData.getCsvData());
	...
	public void testValidateDesignData() throws MiddlewareQueryException{
		...
		try{
			...
			service.validateDesignData(designImportData);
	...
	public void testValidateEntryNoMustBeUniquePerInstance() throws MiddlewareQueryException{
		...
		try {
			...
			DesignHeaderItem trialInstanceHeaderItem = service.filterDesignHeaderItemsByTermId(TermId.TRIAL_INSTANCE_FACTOR, designImportData.getMappedHeaders().get(PhenotypicType.TRIAL_ENVIRONMENT));
			DesignHeaderItem entryNoHeaderItem = service.filterDesignHeaderItemsByTermId(TermId.ENTRY_NO, designImportData.getMappedHeaders().get(PhenotypicType.GERMPLASM));
			
			Map<String, Map<Integer, List<String>>> data = service.groupCsvRowsIntoTrialInstance(trialInstanceHeaderItem, designImportData.getCsvData());
			
			service.validateEntryNoMustBeUniquePerInstance(entryNoHeaderItem, data);
	...
	public void testValidateEntryNoMustBeUniquePerInstanceEntryNoIsNotUnique() throws MiddlewareQueryException{
		...
		Map<String, Map<Integer, List<String>>> data = service.groupCsvRowsIntoTrialInstance(trialInstanceHeaderItem, csvData);
	...
	public void testValidateGermplasmEntriesFromShouldMatchTheGermplasmList(){
		...
		try{
			...
			service.validateGermplasmEntriesFromShouldMatchTheGermplasmList(entryNumbers);
	...
	public void testValidateIfEntryNumberExistsNoEntryNumber() throws MiddlewareQueryException{
		...
		try{
			...
			service.validateIfEntryNumberExists(designImportData.getMappedHeaders().get(PhenotypicType.TRIAL_ENVIRONMENT));
	...
	public void testValidateIfEntryNumberExistsWithEntryNumber() throws MiddlewareQueryException{
		...
		try{
			...
			service.validateIfEntryNumberExists(designImportData.getMappedHeaders().get(PhenotypicType.GERMPLASM));
	...
	public void testValidateIfPlotNumberExistsNoPlotNumber() throws MiddlewareQueryException{
		...
		try{
			...
			service.validateIfPlotNumberExists(designImportData.getMappedHeaders().get(PhenotypicType.GERMPLASM));
	...
	public void testValidateIfPlotNumberExistsWithPlotNumber() throws MiddlewareQueryException{
		...
		try{
			...
			service.validateIfPlotNumberExists(designImportData.getMappedHeaders().get(PhenotypicType.TRIAL_DESIGN));
	...
	public void testValidateIfTrialFactorExists() throws MiddlewareQueryException{
		...
		try{
			...
			service.validateIfTrialFactorExists(designImportData.getMappedHeaders().get(PhenotypicType.TRIAL_ENVIRONMENT));
	...
	public void testValidateIfTrialFactorExistsTrialInstanceDoNotExist() throws MiddlewareQueryException{
		...
		try{
			...
			service.validateIfTrialFactorExists(designImportData.getMappedHeaders().get(PhenotypicType.GERMPLASM));
	...
	private void initializeDesignImportData() throws URISyntaxException, FileParsingException{
		...
		doReturn(file).when(parser).storeAndRetrieveFile(multiPartFile);
		designImportData = parser.parseFile(multiPartFile);
		
	}
	
	private void initializeOntologyService() throws MiddlewareQueryException{
		
		doReturn(createStandardVariable(PhenotypicType.GERMPLASM, TermId.ENTRY_NO.getId(), "ENTRY_NO", "", "", "", "", "", ""))
			.when(ontologyService).getStandardVariable(TermId.ENTRY_NO.getId());
		doReturn(createStandardVariable(PhenotypicType.GERMPLASM, TermId.GID.getId(), "GID", "", "", "", "", "", ""))
			.when(ontologyService).getStandardVariable(TermId.GID.getId());
		doReturn(createStandardVariable(PhenotypicType.GERMPLASM, TermId.DESIG.getId(), "DESIG", "", "", "", "", "", ""))
			.when(ontologyService).getStandardVariable(TermId.DESIG.getId());
		doReturn(createStandardVariable(PhenotypicType.GERMPLASM, TermId.ENTRY_TYPE.getId(), "ENTRY_TYPE", "", "", "", "", "", ""))
			.when(ontologyService).getStandardVariable(TermId.ENTRY_TYPE.getId());
		doReturn(createStandardVariable(PhenotypicType.GERMPLASM, TermId.CROSS.getId(), "CROSS", "", "", "", "", "", ""))
			.when(ontologyService).getStandardVariable(TermId.CROSS.getId());
		doReturn(createStandardVariable(PhenotypicType.GERMPLASM, TermId.ENTRY_CODE.getId(), "ENTRY_CODE", "", "", "", "", "", ""))
			.when(ontologyService).getStandardVariable(TermId.ENTRY_CODE.getId());
		doReturn(createStandardVariable(PhenotypicType.GERMPLASM, TermId.GERMPLASM_SOURCE.getId(), "GERMPLASM_SOURCE", "", "", "", "", "", ""))
			.when(ontologyService).getStandardVariable(TermId.GERMPLASM_SOURCE.getId());
		doReturn(createStandardVariable(PhenotypicType.GERMPLASM, TermId.SEED_SOURCE.getId(), "SEED_SOURCE", "", "", "", "", "", ""))
			.when(ontologyService).getStandardVariable(TermId.SEED_SOURCE.getId());
		
		
		doReturn(createStandardVariable(PhenotypicType.VARIATE, 18000, "GYLD", "", "", "", "N", "", ""))
		.when(ontologyService).getStandardVariable(18000);
		doReturn(createStandardVariable(PhenotypicType.VARIATE, 22768, "CHALK_PCT", "", "", "N", "", "", ""))
		.when(ontologyService).getStandardVariable(22768);
		
		Property prop = new Property();
		Term term = new Term();
		term.setId(0);
		prop.setTerm(term);
		doReturn(prop).when(ontologyService).getProperty(Mockito.anyString());
		
		Map<String, List<StandardVariable>> map = new HashMap<>();
		
		map.put("TRIAL_INSTANCE", createList(createStandardVariable(PhenotypicType.TRIAL_ENVIRONMENT, TermId.TRIAL_INSTANCE_FACTOR.getId(), "TRIAL_INSTANCE", "", "", "", "", "", "")));
		map.put("SITE_NAME", createList(createStandardVariable(PhenotypicType.TRIAL_ENVIRONMENT, TermId.TRIAL.getId(), "SITE_NAME", "", "", "", "", "", "")));
		map.put("ENTRY_NO",createList(createStandardVariable(PhenotypicType.GERMPLASM, TermId.ENTRY_NO.getId(), "ENTRY_NO", "", "", "", "", "", "")));
		map.put("PLOT_NO", createList(createStandardVariable(PhenotypicType.TRIAL_DESIGN, TermId.PLOT_NO.getId(), "PLOT_NO", "", "", "", "", "", "")));
		map.put("REP_NO", createList(createStandardVariable(PhenotypicType.TRIAL_DESIGN, TermId.REP_NO.getId(), "REP_NO", "", "", "", "", "", "")));
		map.put("BLOCK_NO", createList(createStandardVariable(PhenotypicType.TRIAL_DESIGN, TermId.BLOCK_NO.getId(), "BLOCK_NO", "", "", "", "", "", "")));
		
		doReturn(map).when(ontologyDataManager).getStandardVariablesInProjects(Mockito.anyList());
	...
	private void initializeGermplasmList() {
		...
		doReturn(mainInfo).when(userSelection).getImportedGermplasmMainInfo();
	...
	private Map<Integer,StandardVariable> getStandardVariables(PhenotypicType phenotypicType, List<MeasurementVariable> germplasmFactors) {
		...
		for (MeasurementVariable measurementVar : germplasmFactors){
			...
			if (phenotypicType.getLabelList().contains(measurementVar.getLabel())){
				StandardVariable stdVar = convertToStandardVariable(measurementVar);
	...
	protected void createDesignHeaderItemMap(DesignImportData designImportData) throws MiddlewareQueryException{
			...
			designImportData.getMappedHeaders().putAll(service.categorizeHeadersByPhenotype(designImportData.getUnmappedHeaders()));
	...
	protected StandardVariable createStandardVariable(PhenotypicType phenotypicType, int id,String name ,String property, String scale, String method, String dataType,String storedIn, String isA){
