...
public class ExcelExportStudyServiceTest extends AbstractBaseIntegrationTest {
	...
	public void testExportingMoreThan4000Observations() {
		...
		Workbook workbook = WorkbookDataUtil.getTestWorkbook(WorkbookDataUtil.NUMBER_OF_OBSERVATIONS, StudyType.N);
		String filenamePath =
		...
		try {
			...
			org.apache.poi.ss.usermodel.Workbook xlsBookOutput = ExcelImportUtil.parseFile(filenamePath);
			...
			int expectedNoOfRowsInDescriptionSheet = this.getTotalNoOfRowsInDescriptionSheet(workbook, visibleColumns);
	...
	public void testGetFileNamePathWithSiteAndMoreThanOneInstance() {
		...
		ExcelExportStudyServiceTest.fieldbookMiddlewareService =
				Mockito.mock(org.generationcp.middleware.service.api.FieldbookService.class);
		try {
			Mockito.when(ExcelExportStudyServiceTest.fieldbookMiddlewareService.getLocationById(WorkbookDataUtil.LOCATION_ID_1))
					.thenReturn(locations.get(0));
			Mockito.when(ExcelExportStudyServiceTest.fieldbookMiddlewareService.getLocationById(WorkbookDataUtil.LOCATION_ID_2))
					.thenReturn(locations.get(1));
		} catch (MiddlewareQueryException e) {
			ExcelExportStudyServiceTest.LOG.error(e.getMessage(), e);
		...
		excelExportStudyServiceImpl.setFieldbookMiddlewareService(ExcelExportStudyServiceTest.fieldbookMiddlewareService);
		...
		try {
			...
			for (MeasurementRow row : workbook.getTrialObservations()) {
				String outputFileName = excelExportStudyServiceImpl.getFileNamePath(index, row, instances, fileName, false);
	...
	public void testGetFileNamePathWithSiteAndOneInstance() {
		...
		try {
			String outputFileName =
					excelExportStudyServiceImpl.getFileNamePath(1, workbook.getTrialObservations().get(0), instances, fileName, false);
			Assert.assertTrue("Expected location in filename but did not found one.",
