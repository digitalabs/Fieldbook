...
public class ExportAdvanceListServiceImplTest {
	...
	public void setUp() throws MiddlewareQueryException{
		exportAdvanceListServiceImpl = Mockito.spy(new ExportAdvanceListServiceImpl());
		fieldbookProperties = Mockito.mock(FieldbookProperties.class);
		inventoryMiddlewareService = Mockito.mock(InventoryService.class);
		fieldbookMiddlewareService = Mockito.mock(FieldbookService.class);
		exportServiceImpl = Mockito.mock(ExportService.class);
		
		Mockito.when(fieldbookProperties.getUploadDirectory()).thenReturn(tempDirectory);
		GermplasmList germplasmList = new GermplasmList();
		germplasmList.setName("TempGermplasmListName");
		Mockito.when(fieldbookMiddlewareService.getGermplasmListById(Mockito.anyInt())).thenReturn(germplasmList);						
		
		exportAdvanceListServiceImpl.setFieldbookProperties(fieldbookProperties);
		exportAdvanceListServiceImpl.setInventoryMiddlewareService(inventoryMiddlewareService);
		exportAdvanceListServiceImpl.setMessageSource(Mockito.mock(MessageSource.class));
		exportAdvanceListServiceImpl.setFieldbookMiddlewareService(fieldbookMiddlewareService);
		advancedListIds = "1|2|3";
	}
	@Test 
	public void testParseDelimitedAdvanceGermplasmListIds(){
		List<Integer> advanceIds = exportAdvanceListServiceImpl.parseDelimitedAdvanceGermplasmListIds(advancedListIds);
	...
	public void testGetInventoryAmountWhenAmountIsNull(){
		InventoryDetails inventoryDetails = getSampleInventoryDetails(1,2);
		inventoryDetails.setAmount(null);
				
		Assert.assertEquals("The Amount should return be empty string", "", exportAdvanceListServiceImpl.getInventoryAmount(inventoryDetails));		
	}
	...
	public void testGetInventoryAmountWhenAmountIsNonNull(){		
		...
		Assert.assertEquals("The Amount should be 2.22", "2.22", exportAdvanceListServiceImpl.getInventoryAmount(inventoryDetails));		
	}
	...
	public void testGetInventoryScaleWhenScaleIsNull(){
		...
		Assert.assertEquals("The Scale should return be empty string", "", exportAdvanceListServiceImpl.getInventoryValue(inventoryDetails.getScaleName()));
	}
	...
	public void testGetInventoryScaleWhenScaleIsNonNull(){		
		...
		Assert.assertEquals("The Scale should be "+scaleName, scaleName, exportAdvanceListServiceImpl.getInventoryValue(inventoryDetails.getScaleName()));		
	}
	...
	public void testGetInventoryCommentWhenCommentIsNull(){
		...
		Assert.assertEquals("The Comment should return be empty string", "", exportAdvanceListServiceImpl.getInventoryValue(inventoryDetails.getComment()));
	}
	...
	public void testGetInventoryCommentWhenCommentIsNonNull(){		
		...
		Assert.assertEquals("The Comment should be "+comment, comment, exportAdvanceListServiceImpl.getInventoryValue(inventoryDetails.getComment()));		
	}
	...
	public void testGetFileNamePath(){
		...
		String fileNamePath = exportAdvanceListServiceImpl.getFileNamePath(fileName);
	...
	public void testZipFileNameList() throws IOException{
		...
		exportAdvanceListServiceImpl.zipFileNameList(fileZip.getName(), filenameList);
	...
	public void testExportAdvanceGermplasmListInCsvMoreThan1AdvanceItem() throws IOException{
		Mockito.doReturn(true).when(exportAdvanceListServiceImpl).zipFileNameList(Mockito.anyString(), Mockito.anyList());
		Mockito.doReturn("TempGermplasmListName").when(exportAdvanceListServiceImpl).getFileNamePath(Mockito.anyString());
		Mockito.when(exportServiceImpl.generateCSVFile(Mockito.anyList(), Mockito.anyList(), Mockito.anyString())).thenReturn(new File("Temp"));
		
		File file = exportAdvanceListServiceImpl.exportAdvanceGermplasmList(advancedListIds, studyName, exportServiceImpl, AppConstants.EXPORT_ADVANCE_NURSERY_CSV.getString());
	...
	public void testExportAdvanceGermplasmListInCsvOnly1Item() throws IOException{
		...
		File file = exportAdvanceListServiceImpl.exportAdvanceGermplasmList("1", studyName, exportServiceImpl, AppConstants.EXPORT_ADVANCE_NURSERY_CSV.getString());
	...
	public void testExportAdvanceGermplasmListInCsvThrowsIOException() throws MiddlewareQueryException, IOException{
		Mockito.doReturn("TempGermplasmListName").when(exportAdvanceListServiceImpl).getFileNamePath(Mockito.anyString());
		Mockito.when(exportServiceImpl.generateCSVFile(Mockito.anyList(), Mockito.anyList(), Mockito.anyString())).thenThrow(new IOException());
		File file = exportAdvanceListServiceImpl.exportAdvanceGermplasmList("1", studyName, exportServiceImpl, AppConstants.EXPORT_ADVANCE_NURSERY_CSV.getString());
	...
	public void testExportAdvanceGermplasmListInCsvThrowsMiddlewareException() throws MiddlewareQueryException, IOException {
		Mockito.doReturn("TempGermplasmListName").when(exportAdvanceListServiceImpl).getFileNamePath(Mockito.anyString());
		Mockito.when(fieldbookMiddlewareService.getGermplasmListById(Mockito.anyInt())).thenThrow(new MiddlewareQueryException("error"));
	...
	public void testExportAdvanceGermplasmListInXlsOnly1Item() throws IOException{
		Mockito.doReturn("TempGermplasmListName").when(exportAdvanceListServiceImpl).getFileNamePath(Mockito.anyString());
		Mockito.when(exportServiceImpl.generateExcelFileForSingleSheet(Mockito.anyList(), Mockito.anyList(), Mockito.anyString(), Mockito.anyString())).thenReturn(new FileOutputStream(new File("temp")));
		
		File file = exportAdvanceListServiceImpl.exportAdvanceGermplasmList("1", studyName, exportServiceImpl, AppConstants.EXPORT_ADVANCE_NURSERY_EXCEL.getString());
	...
	public void testExportAdvanceGermplasmListInXlsThrowsIOException() throws IOException{
		Mockito.doReturn("TempGermplasmListName").when(exportAdvanceListServiceImpl).getFileNamePath(Mockito.anyString());
		Mockito.when(exportServiceImpl.generateExcelFileForSingleSheet(Mockito.anyList(), Mockito.anyList(), Mockito.anyString(), Mockito.anyString())).thenThrow(new IOException());
	...
	public void testExportAdvanceGermplasmListInXlsMoreThan1AdvanceItem() throws IOException{
		...
		File file = exportAdvanceListServiceImpl.exportAdvanceGermplasmList(advancedListIds, studyName, exportServiceImpl,  AppConstants.EXPORT_ADVANCE_NURSERY_EXCEL.getString());
		Assert.assertTrue("Return should be a zip file", file.getAbsolutePath().indexOf(".zip") != -1);
	}
	...
	private List<InventoryDetails> generateSampleInventoryDetailsList(int rows){
		...
		for(int i = 0 ; i < rows ; i++){
			inventoryDetailList.add(getSampleInventoryDetails(i, i));
		}
