...
public class GenerateFieldmapControllerTest {
	...
	public void testExportExcel() throws Exception {
		...
		when(exportExcelService.exportFieldMapToExcel(anyString(),eq(userFieldmap))).thenReturn(mock(
		...
		assertTrue("Content-disposition value has the filename argument",argument.getValue().contains("filename=\"" + fileName + "\""));
	}
	...
	public void makeSafeFileName() throws Exception {
		...
		assertTrue("Contains the BLOCK_NAME without spaces", out.contains(BLOCK_NAME.replace(" ","")));
		assertTrue("No spaces, ends with \"-<current_date>.xls\"",!out.contains(" ") && out.endsWith("-" + DateUtil.getCurrentDate() + ".xls"));
