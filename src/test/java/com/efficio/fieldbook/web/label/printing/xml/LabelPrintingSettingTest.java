...
public class LabelPrintingSettingTest {
	...
	public void testPDFOutputXMLConfiguration() throws JAXBException{
		String rightFieldsString = stringify(SELECTED_FIELDS_2, PDFLabelPrintingSetting.DELIMITER);
		String leftFieldsString = stringify(SELECTED_FIELDS_1, PDFLabelPrintingSetting.DELIMITER);
		String barcodeFieldsString = stringify(BARCODE_FIELDS, BarcodeLabelPrintingSetting.DELIMITER);
		...
		LabelPrintingSetting labelPrintingSetting = new LabelPrintingSetting("PDF setting", "PDF", 
				null, pdfSetting, barcodeSetting);
        ...
        String xmlToRead = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><labelPrintingSetting name=\"PDF setting\" outputType=\"PDF\">"
        		+ "<barcodeSetting barcodeFieldsString=\"" + barcodeFieldsString + "\" barcodeFormat=\"Barcode\" barcodeNeeded=\"true\"/> "
        ...
        Assert.assertEquals(Arrays.asList(SELECTED_FIELDS_1), parsedSetting.getPdfSetting().getSelectedLeftFieldsList());
        Assert.assertEquals(Arrays.asList(SELECTED_FIELDS_2), parsedSetting.getPdfSetting().getSelectedRightFieldsList());
        Assert.assertEquals(Arrays.asList(BARCODE_FIELDS), parsedSetting.getBarcodeSetting().getBarcodeFieldsList());
        
	}
	
	@Test
	public void testCSVExcelOutputXMLConfiguration() throws JAXBException{
		String selectedFieldsString = stringify(SELECTED_FIELDS_1, PDFLabelPrintingSetting.DELIMITER);
		String barcodeFieldsString = stringify(BARCODE_FIELDS, BarcodeLabelPrintingSetting.DELIMITER);
		
		CSVExcelLabelPrintingSetting csvExcelSetting = new CSVExcelLabelPrintingSetting(false, selectedFieldsString);
		BarcodeLabelPrintingSetting barcodeSetting = new BarcodeLabelPrintingSetting(true, "Barcode", barcodeFieldsString);
		LabelPrintingSetting labelPrintingSetting = new LabelPrintingSetting("PDF setting", "PDF", 
				csvExcelSetting, null, barcodeSetting);
        ...
        Assert.assertEquals(labelPrintingSetting, parsedSetting);
        Assert.assertEquals(Arrays.asList(SELECTED_FIELDS_1), parsedSetting.getCsvExcelSetting().getSelectedFieldsList());
        Assert.assertEquals(Arrays.asList(BARCODE_FIELDS), parsedSetting.getBarcodeSetting().getBarcodeFieldsList());
	}
