...
public class AdvancingControllerTest {
	...
	public void testDeleteImportedGermplasmEntriesIfDeleted() {
		...
		this.advancingController.updateAdvancingSource("1", importedGermplasms);
		Assert.assertEquals("Should have a total of 7 germplasms remaining", 7, importedGermplasms.size());
		Assert.assertEquals("ImportedGermplasm Should be equal to Advancing Source Items", importedGermplasms.size(), this.advancingController.getPaginationListSelection().getAdvanceDetails("1").getAdvancingSourceItems().size());
	...
	public void testGenerateGermplasmList() throws MiddlewareQueryException {
		...
		ig.setGpid2(0);
		ig.setGnpgs(-1);
		final AdvancingSourceList rows = this.getAdvancingSourceList(Collections.singletonList(ig));
	...
	private AdvancingSourceList getAdvancingSourceList(final List<ImportedGermplasm> germplasmList) {
		...
		for (final ImportedGermplasm germplasm : germplasmList) {
			...
			advancingSource.setGermplasm(germplasm);
